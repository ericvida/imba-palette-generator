...
class ColorCard
	...
	@sat = 80
	@lit = 90
	def values hue, sat, lit, high, mid, low, steps
		@bg = "hsl({@hue},{@sat}%,{@lit}%)"
		@stepsBetween = (steps - 1) / 2 # 4
		@highStepSize = (high - mid) / @stepsBetween # 5
		@lowStepSize = (mid - low) / @stepsBetween # 10
		# make array using steps
		@litSteps = [low]
		for item in [low..mid]
			until low == mid
				@litSteps.push(low += @lowStepSize)
		for item in [mid ... high]
			until mid == high
				@litSteps.push(mid += @highStepSize)
		
		@allSteps = ''

let yellow = ColorCard.new
yellow.values(45,90,60,90,70,30,stepNum)
console.log yellow.sat
tag color-card-b
	def render
		<self>
			<input[yellow.hue] type="range" min="0" max="360">
			<h1 style="color: hsl({yellow.hue},100%,60%);"> "hue: {@hue}"
### css
###
tag app-root
	@theme = [
